...
#SBATCH --job-name=vim-s-cifar-alternate-merge-r14          # 作业名称
#SBATCH --account=PAS2490		    # Project ID
#SBATCH --output=/users/PAS2490/marcusshen/Vim/output_logs_vim/vim-s-cifar-alternate-merge-r14.log         # 输出日志文件
#SBATCH --error=/users/PAS2490/marcusshen/Vim/output_logs_vim/vim-s-cifar-alternate-merge-r14_error.log           # 错误日志文件
#SBATCH --nodes=1                   # 节点数
#SBATCH --ntasks-per-node=1         # 每个节点的任务数
#SBATCH --cpus-per-task=4           # 每个任务使用的 CPU 核心数
#SBATCH --gpus-per-node=4	        # GPU per node
#SBATCH --mem=80G                   # 内存限制
#SBATCH --time=35:00:00             # 作业运行时间限制
