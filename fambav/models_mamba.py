...
from token_merge import bipartite_soft_matching
from token_merge import kth_bipartite_soft_matching
...
def visualize_cosine_similarity(hidden_states):
    print("Start to visualize cosine similarity")
    # 获取hidden_states的形状
    batch_size, seq_len, hidden_dim = hidden_states.shape

    # 将hidden_states重塑为二维数组
    hidden_states_reshaped = hidden_states.reshape(batch_size * seq_len, hidden_dim)

    if isinstance(hidden_states_reshaped, torch.Tensor):
       hidden_states_reshaped = hidden_states_reshaped.detach().cpu().numpy()

    print("hidden_states_reshaped.shape: ", hidden_states_reshaped.shape)
    # 计算余弦相似度矩阵
    cosine_sim_matrix = cosine_similarity(hidden_states_reshaped)
    
    print("cosine_sim_matrix success")
    # 使用 seaborn 绘制热图
    plt.figure(figsize=(10, 8))
    sns.heatmap(cosine_sim_matrix, cmap='viridis')
    plt.title("Cosine Similarity Matrix of Hidden States")
...
class Block(nn.Module):
    def __init__(
        self, dim, mixer_cls, token_merge=False, visualize=False, fusion_token=0, norm_cls=nn.LayerNorm, fused_add_norm=False, residual_in_fp32=False,drop_path=0.,
    ):
        """
    ...
    def forward(
        ...
        if self.visualize:
            # visualize_cosine_similarity(hidden_states)
            self.hidden_states_np = hidden_states[0].detach().cpu().numpy()
        ...
        #     if r > 0:
        #         # merge, unmerge = kth_bipartite_soft_matching(hidden_states, r)
        #         merge, unmerge = bipartite_soft_matching(hidden_states, r, class_token=True)
        #         hidden_states = merge(hidden_states)
        #         residual = merge(residual) if residual is not None else None
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        for i in range(depth):
            if ((self.fusion_strategy == 'lower' and i > self.fusion_layer) or
                (self.fusion_strategy == 'upper' and i < self.fusion_layer) or
                (self.fusion_strategy == 'alternate' and i % 2 == 1) or
                (self.fusion_strategy == 'all' and i >= 0)):
                created_block = create_block(
                    embed_dim,
                    token_merge=True,
                    visualize=self.visualize,
                    fusion_token=self.fusion_token,
                    ssm_cfg=ssm_cfg,
            ...
            blocklayer.append(created_block)
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if not self.if_bidirectional:
            for layer in self.layers:
                ...
                if self.visualize:
                    hidden_states_np_list.append(layer.hidden_states_np)
        else:
            ...
            for i in range(len(self.layers) // 2):
                ...
                if self.bi_merge:
                    if ((self.fusion_strategy == 'upper' and i > self.fusion_layer) or
                        (self.fusion_strategy == 'lower' and i < self.fusion_layer) or
                        (self.fusion_strategy == 'alternate' and i % 2 == 1) or
                        (self.fusion_strategy == 'all' and i >= 0)):
                        t = hidden_states.shape[1]
                        r = self.fusion_token
                ...
                if self.visualize:
                    hidden_states_np_list.append(self.layers[i * 2].hidden_states_np)
        
        if self.visualize: 
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"./visualization/hidden_states_{timestamp}"
            np.savez(filename, **{f'layer_{i}': array for i, array in enumerate(hidden_states_np_list)})
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=False, if_bidirectional=True, **kwargs)
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=False, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
